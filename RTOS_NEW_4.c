...
int main(void)
{
  ...
  xTaskCreate(vManagerTask, "Manager", 200, NULL, 3, NULL);
  xTaskCreate(vEmployeeTask, "Employee", 200, NULL, 1, NULL);
...
static void vManagerTask(void* pvParmaeters){
	...
	while(1){
		...
		if(xStatus != pdPASS){
			printf("error in sending to queue - manager task");
		}
		else{
			xSemaphoreGive(xWork);
			taskYIELD();
...
static void vEmployeeTask(void* pvParmaeters){
	unsigned int xWorkTicketId;
	portBASE_TYPE xStatus;
	while(1){
		...
		if(xStatus == pdPASS){
			...
			printf("message not recived - employee task");
		}
