...
TaskHandle_t sender_handle , reciever_handle;
...
int main(void)
{
  ...
  yearQueue = xQueueCreate(5,sizeof(uint32_t));
   xTaskCreate(SenderTask,"sender_task",256,NULL,1,sender_handle);
   xTaskCreate(RecieverTask,"reciever_task",256,NULL,1,reciever_handle);
   vTaskStartScheduler();
  /* add queues, ... */
...
void RecieverTask(void *pvParameters){
	int32_t value_recieved;
	const TickType_t wait_time = pdMS_TO_TICKS(100);
	BaseType_t qStatus;
	while(1){
		qStatus =  xQueueReceive(yearQueue, &value_recieved, wait_time);
		if(qStatus == pdPASS){
			printf("value recieved is %ld \r\n",value_recieved);
			vTaskDelay(pdMS_TO_TICKS(500));
		}
		else{
